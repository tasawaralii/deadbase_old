...
$pack_query = "SELECT drive_id, quality, size FROM Packs WHERE my_season_id = $my_season_id ORDER BY `Packs`.`size` ASC";
...
echo "<strong><center><a href='https://drive.deadtoons.online/file/{$pack_row['drive_id']}' target='_blank'>{$pack_row['quality']} - [" . formatSize($pack_row['size']) . "]</a></center></strong><hr>";
     }
}
mysqli_close($conn);
?>